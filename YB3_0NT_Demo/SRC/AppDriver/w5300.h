...
/***   			2、该文件定义了W5300寄存器的内存映射，也定义了基本的I/O功能访问W5300的寄存器。***********/
/********************************************************************************************************/
#ifndef	_W5300_H_
#define	_W5300_H_
...
//W5300端口宏定义
#define W5300_SOCKET0  0x00	//JG0,垂直激光接收			
#define W5300_SOCKET1  0x01 //JG1,倾斜激光接收
#define W5300_SOCKET2  0x02	//JG2,模拟波形上位机接收
#define W5300_SOCKET3  0x03	
#define W5300_SOCKET4  0x04
#define W5300_SOCKET5  0x05
#define W5300_SOCKET6  0x06
#define W5300_SOCKET7  0x07 

/* 端口的运行状态 */
//#define S_INIT	0x01				/* 端口完成初始化 */
//#define S_CONN	0x02				/* 端口完成连接，可以正常传输数据 */

//使用16为数据模式，ADDR0 内部下拉。MR有效，MR0,MR1无效。
/**Mode register*/
#define MR              (__DEF_W5300_MAP_BASE__)
//#define MR0             MR			 
//#define MR1             (MR + 1)	//STR710此处为 + 1 ,STM32此处为 + 2

/**Indirect mode address register*/ 
#define IDM_AR          (__DEF_W5300_MAP_BASE__ + 0x02)
//#define IDM_AR0         IDM_AR
//#define IDM_AR1         (IDM_AR + 1)

/**Indirect mode data register */ 
#define IDM_DR          (__DEF_W5300_MAP_BASE__ + 0x04)
...
#define IMR_NET             (COMMON_REG_BASE + 0x04)
//#define IMR0            IMR_NET
//#define IMR1            (IMR_NET + 1)
...
#define IPPROTO_IGMP       2   /* Inter group management protocol */
...
void W5300SetPara(uint32 startaddr,uint8 *value,uint8 len);
...
//extern void W5300_Socket_Init(SOCKET s, uint8 protocol, uint16 port, uint16 flag);
//extern void W5300_loopback_tcps(SOCKET s, uint16 port, uint8* buf,uint16 mode);
//extern uint8 W5300_State_management(SOCKET s, uint16 port, uint16 mode,uint8 *JG_IP,uint16 JG_DPort);
//extern void W5300_loopback_udp(SOCKET s, uint16 port, uint8* buf, uint16 mode);
//extern uint32   W5300_Socket_Recv(SOCKET s, uint8 * buf, uint32 len);
//uint8 Detect_Gateway(SOCKET s);		  //内部使用

extern uint8 W5300_GATEWAY[4] ;     /* 网关IP地址 */
extern uint8 W5300_SUBNET[4] ;		 /* 子网掩码 */
extern uint8 W5300_MAC_ADDR[6] ;	 /* 物理地址 */
extern uint8 W5300_IP_ADDR[4] ;	 /* 本机IP地址 */

/*****目的IP地址*****/
extern uint8  S0_IP[4];
extern uint8  S1_IP[4];
extern uint8  S2_IP[4];
extern uint8  S3_IP[4];
extern uint16 W5300_source_port;
extern uint8 W5300_InterruptFlag;

/*****目的端口号*****/
extern uint16 S0_DPort;	   /*垂直激光*/
extern uint16 S1_DPort;	   /*倾斜激光*/
extern uint16 S2_DPort;	   /*上位机发送*/
extern uint16 S3_DPort;	   /*上位机发送*/
/*****源端口号*****/
extern uint16 S0_Port;	//4000
extern uint16 S1_Port;	//4001
extern uint16 S2_Port;	//4002
extern uint16 S3_Port;	//4002

/*****各端口协议模式*****/
extern uint8 S0_Mode;//
extern uint8 S1_Mode;//
extern uint8 S2_Mode;//
extern uint8 S3_Mode;//

/**	 * The information of w5300	*/ 

/** The variables for SOCKETn interrupt */
extern volatile uint8 SOCK_INT[MAX_SOCK_NUM]; 
 ...
 */
uint8 TX_RX_Buf_Config(uint8* tx_size, uint8* rx_size); // setting tx/rx buf size
