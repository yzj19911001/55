...
void Task_Uart1Rec(void *tdata)
{	
	uint8 err=0;
	tdata=tdata;
	while(1)
	{
		OSSemPend(g_Uart1Recv,0,&err);	 		  
		OSTimeDly(30);
		g_Uart1Recv->OSEventCnt = 0;
		RecCom1Data();
	}	
}
...
uint8 RecCom1Data(void)
{
	uint8 U1RecBuff[UARTRCVBUFSIZE]={0};
	uint16 index = 0;
	...
	temp_head =  g_UART1_Proc_Index;
	temp_tail =  g_UART1_Recv_Index;
	if(temp_head != temp_tail )		 //缓存队列不为空
	{
		...
		{
		  	if(g_u8U1RcvBuf[temp_head] == 0xFF && (g_u8U1RcvBuf[(temp_head+1)&(UARTRCVBUFSIZE-1)] == 0xFF) )	  //判断帧头
			{
			   if( temp_length >= MIN_FRAME_LENGTH )  //待处理缓存长度大于最小包长7   能使用 g_u8U1RcvBuf[ (temp_head+3)%1024 ]
			   {
			   		if( (g_u8U1RcvBuf[(temp_head + 2)&(UARTRCVBUFSIZE-1) ] == 0x80 || g_u8U1RcvBuf[(temp_head + 2)&(UARTRCVBUFSIZE-1) ] == 0x81 )&& g_u8U1RcvBuf[(temp_head + 3)&(UARTRCVBUFSIZE-1) ] == 0x3B)
					{ //寻找结束FF
						if( temp_length > 15 )	//寻找结束FF,长度可以包含结束的FF
						{
							frame_length = 15;
							while( g_u8U1RcvBuf[((temp_head + frame_length)&(UARTRCVBUFSIZE-1)) ] != 0xFF &&  ((temp_head + frame_length)&(UARTRCVBUFSIZE-1)) != temp_tail )  //寻找后续的第一个FF
								  frame_length++;
						}
						...
						if( g_u8U1RcvBuf[(temp_head + frame_length)&(UARTRCVBUFSIZE-1) ] == 0xFF  && temp_length < MAX_FRAME_LENGTH_FF)	//寻找到FF
						{
							frame_length = frame_length +1;
								memset(U1RecBuff , 0 , UARTRCVBUFSIZE);
							for(index = 0; index <frame_length ;index++ )//适应后续的解析函数，需要提取出有效数据
							{
								U1RecBuff[index] =  g_u8U1RcvBuf[ (temp_head+index)% UARTRCVBUFSIZE ]	;
							}
						}
						else if	( g_u8U1RcvBuf[(temp_head + frame_length)&(UARTRCVBUFSIZE-1) ] != 0xFF  && temp_length < MAX_FRAME_LENGTH_FF )
							frame_length = temp_length+1 ;	//等待下一次接收完成
						else
							frame_length = 	MAX_FRAME_LENGTH_FF + 1;	
					}
					else if(g_u8U1RcvBuf[(temp_head + 2)&(UARTRCVBUFSIZE-1) ] == 0x82 && g_u8U1RcvBuf[(temp_head + 3)&(UARTRCVBUFSIZE-1) ] == 0x3B)
					{
						...
						if( U1RecBuff[7] != 0xFF )  //无效帧
						{
							frame_length = 	MAX_FRAME_LENGTH_FF + 1;//舍弃当前帧头
						}
					}
					else if(  g_u8U1RcvBuf[(temp_head+2)&(UARTRCVBUFSIZE-1)] == 0x02 &&  g_u8U1RcvBuf[(temp_head+3)&(UARTRCVBUFSIZE-1)] == 0x02 && g_u8U1RcvBuf[(temp_head+4)&(UARTRCVBUFSIZE-1)] == 0x02 && g_u8U1RcvBuf[(temp_head+5)&(UARTRCVBUFSIZE-1)] == 0x02)
					{	//激光透传数据
						if( ( temp_length >= MIN_FRAME_LENGTH_TC ))						
						{
							frame_length =(g_u8U1RcvBuf[ (temp_head+6)&(UARTRCVBUFSIZE-1) ]<<24) + (g_u8U1RcvBuf[ (temp_head+7)&(UARTRCVBUFSIZE-1) ]<<16) +(g_u8U1RcvBuf[ (temp_head+8)&(UARTRCVBUFSIZE-1) ]<<8) + g_u8U1RcvBuf[(temp_head+9)&(UARTRCVBUFSIZE-1)] +18 ; //求数据包长度，包含头尾校验
						   if(temp_length >= (frame_length) && frame_length <= MAX_FRAME_LENGTH_TC )	 //	 是否包含完整一包  frame_length
						   {
						   		if( g_u8U1RcvBuf[(temp_head+frame_length-1)&(UARTRCVBUFSIZE-1)] == 0xEE )	  //判断帧尾校验
								{	
									memset(U1RecBuff , 0 , UARTRCVBUFSIZE);
									...
									g_UART1_Proc_Index = temp_head;		//队列头赋值
									g_LaserPass.PassCH=CH_COM1;//透传通道为COM5
									PassThroughToJG(U1RecBuff+2);			   //处理数据
							   	}
								else	//不是完整的一包 继续寻找帧头
								{
					...
					{	
						if(g_u8U1RcvBuf[ (temp_head+2)&(UARTRCVBUFSIZE-1) ]==0xFF)
					   {
						...
						//正常交互数据
					   frame_length = (g_u8U1RcvBuf[ (temp_head+3)&(UARTRCVBUFSIZE-1) ]<<8) + g_u8U1RcvBuf[(temp_head+4)&(UARTRCVBUFSIZE-1)]; //求数据包长度，包含头尾校验
					   if(temp_length >= (frame_length) && frame_length <= MAX_FRAME_LENGTH )	 //	 是否包含完整一包  frame_length
					   {
					   		if( CheckCrc(g_u8U1RcvBuf+temp_head,frame_length-2)== 1 )	  //判断帧尾校验
							{	
								memset(U1RecBuff , 0 , UARTRCVBUFSIZE);
								for(index = 0; index <frame_length + 6;index++ )//适应后续的解析函数，需要提取出有效数据
								{
									U1RecBuff[index] =  g_u8U1RcvBuf[ (temp_head+index)&(UARTRCVBUFSIZE-1) ]	;
								}
								AnalyzeCom1Data(U1RecBuff, frame_length);			   //处理数据
						   		temp_head = (frame_length + temp_head )&(UARTRCVBUFSIZE-1);  //确定新的处理起始点;
								temp_length =  temp_length -frame_length ;
						   	}
							else	//不是完整的一包 继续寻找帧头
				   ...
				   {
				   		if (!PktDecode(U1RecBuff ,frame_length,&g_u8FrmNo,g_u8ParseBuf,&g_u32ParseLen  ))//校验成功
						{
		...
		if(temp_length == 1 && g_u8U1RcvBuf[temp_head] != 0xFF )	    //剩下的一个字节不是FF
		{
...
//串口5协议解析
void AnalyzeCom1Data(uint8 *pUartBuf, uint16 len)
{
	uint8 CmdCode=0;
	uint8 CH=CH_COM1;
	uint8 PTRL=PTRL_FF;
	...
	switch(CmdCode)
	{
		...
		case DLL_MANCLOSEOUT:
			DllReplyManOut(CH,PTRL);
			break;
		case DLL_CLOSECHANEL:
			DllCloseCHannel(CH,PTRL);
			break;
		case DLL_ADD1VEHICLE:
			DllAdd1VehInfo(pUartBuf,CH,PTRL);
			break;	
		case DLL_DEL1VEHICLE:
			DllDel1VehInfo(pUartBuf,CH,PTRL);
			break;
		case DLL_UPLDQUEINFO:
			DllUpLoadQueInfo(pUartBuf,CH,PTRL);
			break;			
		default:
			break;
	}
